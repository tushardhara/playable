...
World section: 950–1,175 whitespace-separated words, including headings, ladder contracts, and opening choices. The top of the band is exactly what the block ceilings below sum to; move a ceiling and re-add them. Use the bounded layout below; derivation calculations and edit instructions remain outside the world and need not be emitted. Preserve every world fact when emitting; do not discover a budget problem only after promising an overlong card. PROFILE: at most 100 words, counted outside the world band, never taken from it. It is a list of values, not prose; if it needs explaining it has become a second rule source. Runtime: copy the template below verbatim apart from the six closed markers, which are substituted at emission. Total FILE 2: maximum 3,500 words; do not raise either cap. Count the runtime with every marker at its longest, not as written: the strict clue sentence is 25 words where the marker is one, and the two-chapter quiet quota five where it is one, so the emitted template's worst case is 2,196 words, not 2,168. Worst case overall — world at the 1,175 ceiling, Q8 strictly, no warm tone, PROFILE at 100 — is 3,471 of 3,500, leaving 29. Nothing may be added without a cut, and any new rule must be priced at its substituted length. Publish actual prose/choice metering at this size before claiming it works. If the budget cannot fit, shorten world exposition, never silently drop a runtime rule or closure. These are experimental budgets, not a proven cognitive limit.
...
Ceilings sum to 1,175, exactly the top of the band, so a card drafted to every ceiling is still inside it. Re-add them whenever one moves: revision 7 shipped a table summing fifty words below its own band, and revision 8's first pass summed ten above it. Each rung took fifteen words for the objects it now names and the reserves fifteen for the third reserve the cast floor requires; the nine blocks that moved down gave eighty back between them, the active-cast block did not move, and the band's top rose twenty-five. They are drafting guidance, not a gate: structural ceilings and total arithmetic prove neither compliance nor play metering, and a word count cannot establish that a world is any good. Keep the exact run instruction first, the "Time anchors" heading, and every rung's literal row and six indented labels; do not rename those lines. Put no extra world prose outside these blocks. Draft each block once in its listed home, keep the world within 950–1,175, and cut repeated explanation rather than required facts, causal links, or consequences; never defer repair to emission. Unused space elsewhere cannot excuse a block running long. Report any exact count only if it was actually counted; otherwise mark it unverified in derivation notes outside the world. No tool is required.
...
**CHECK.** Hooks: threat, overreach, arrival, countdown, emotional turn, revelation, quiet. Seven fixed names; `end` is reserved; never rename or add one. No consecutive repeats; quiet [P:quiet_quota]. Copy previous hook/by into last fields; `by` names whose act moved conflict. `took` copies one complete object text after `gone: ` from a new ledger entry this turn, character for character. No loss: dash. `open` lists unsettled promises as id plus chapter-and-turn, oldest first; none: dash.
