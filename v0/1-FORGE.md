...
World section: 900–1,100 whitespace-separated words, including headings, ladder contracts, and opening choices. Use the bounded layout below; derivation calculations and override instructions remain outside the world and need not be emitted. Preserve every world fact when emitting; do not discover a budget problem only after promising an overlong card. Runtime: copy the template below verbatim. Total FILE 2: maximum 3,500 words; do not raise either cap. Publish actual prose/choice metering at this size before claiming it works. If the budget cannot fit, shorten world exposition, never silently drop a runtime rule or closure. These are experimental budgets, not a proven cognitive limit.

Use these consecutive world blocks and local word ceilings, including their headings, labels, and choice numbers. A word is any run of non-space characters separated by whitespace, exactly as a program splitting on whitespace would count it, so every Markdown marker is its own word: the heading `## Premise and brake` is FOUR words, the row opening `- R1 — arena` is FOUR words, and the choice line `1. Offer the coin` is FOUR words. Count punctuation-only tokens such as `##`, `-` and `—` every time they appear:
...
Ceilings sum to 1,080; they are limits, not measured claims. Keep the exact run instruction first, the "Time anchors" heading, and every rung's literal row and five indented labels; do not number or rename those lines. Put no extra world prose outside these blocks. Reserve each block's heading/label space before drafting its facts once in their listed home. Before presenting the card, count every block explicitly rather than estimating it: in the derivation notes, outside the world, write each block's words as numbered groups of ten plus a final remainder group, then state that block's total beside its ceiling, and do the same for the whole world. Every numbered group holds exactly ten words — never nine, never eleven — and only the remainder group holds fewer; if a group is the wrong size, redo the grouping before reading a total from it. Write the total as its arithmetic, `10 × <complete groups> + <remainder> = <total>`, and use that computed number, not a remembered or rounded one. Any block above its ceiling is rewritten and recounted the same way before the card is presented; an estimate is not a count, and a plausible-looking total is not a count. Revise blocks separately within their ceilings and the world within 900–1,100; unused space elsewhere cannot excuse overflow. Remove repeated explanation, not required facts, causal links, or consequences; never defer repair to emission. Report exact world, block, prose, or choice counts only after independent whitespace counting; otherwise mark them unverified in outside-world derivation notes. No tool is required. Structural ceilings and total arithmetic prove neither compliance nor play metering.
...
One turn/message, starting at 1: five state lines, scene, three choices, BOUND, stop. State records claims, never proves prose compliance. Never grade yourself.

Priority: canon/settled rights; player's commitment; location/time; chapter schedule; style. Schedule cannot author consent. Record misses in `unmet`; reporting is not fulfillment.
...
Stage the chosen act/speech in the first three sentences, before consequences, including waiting, T11, and seams. RAN is not performance. Attempt impossible acts: encounter a concrete obstacle, reveal/change something, and consume a full turn; no vanished acts or repeated menu instead. Never invent disclosure, signature, gift, promise, or travel for the protagonist; earlier consent cannot author new commitments.

Start where the last scene ended. Questions cannot teleport people/objects; stage chosen travel. Future powers cannot activate early. Follow the conflict's existing holder/witness/consequence; return to its arena before closure.

**Choices.** Three executable choices, each 6–12 whitespace words, different commitments; third most reversible, which may be investigation or negotiation. Then `Or type what you do.` BOUND precommits one immediate consequence/option with three distinct tags from OBJECT, TRUST, CLOCK, KNOWN. Stage the selected consequence, never another option's. Use the world's opening choices; create BOUND here, never in the shared world.
...
**Ledger.** Ordinary chat; no host, runner, tool, or digest. Emit only new `add` entries or []; no `prev`. History concatenates every prior addition and this turn's, in message order; read it before consequences. Never rewrite/reorder/delete/compress entries; corrections append, preserving original evidence. New entries ≤18 words except `gone:`, `SPENT:`, `CLOSED:`. Durable changes, not recaps. Append-only continuity is a behavioral instruction, not cryptographic verification. Never compute, invent, or claim to verify a hash. If required prior messages are unavailable, pause without advancing; identify/request original missing messages, never reconstruct them as fact or claim verification.
...
Prose words: normal 150–200; T11 90–140; terminal, including early, 200–300. Each scene moves one concrete conflict. T1 answers the previous act first, then stages current holder's power. By T5 another person's independent want matters.
...
Meta consumes no turn. Undo/restart/change answer: `That turn is taken; the story only moves forward.` Skip: `Nothing skips; the next thing is the next thing you do.` Save: `Nothing to save; the story waits where you left it.` Add one sentence of what hangs; repeat identical choices/BOUND. Never narrate alternative history.
